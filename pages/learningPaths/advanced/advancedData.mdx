...
**Step by Step Guide: How to Create New Variables**

The **Calculate Variable** pipeline step let you create new variables. It is an all-purpose formula parser to create new variables in a more "Excelish" way.

01. Fetch Alphacast Dataset

_In this example equity data from Yahoo Finance is used by adding the name of the tickers for Microsoft, Apple, and Google. Select the columns to be used, such as **Close**_


02. Add the **Calculate Variable** step

There is an extensive library of formulas that can be used, including operators, logical and conditional formulas, row and grouping functions, and more. Add formulas by typing the name and opening the parenthesis. But let's begin with an easy example

Exmample 1: Use the AT sign to make reference to variables in the dataset. Then write a simple formula using an asterisk and two to multiply. Et voilá! Now you have the serie multiplied by two.

Example 2: Add another **Calculate variable** step for this new exercise. To add the square root of Close, you'll have to write **sqrt(@close)**. Use nested formulas to replace Close with the square of Close by typing **sqrt(power(@close,2))**. Click save to see a preview.
